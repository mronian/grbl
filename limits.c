...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  bool approach = true;
  float modified_homing_feed_rate, modified_homing_seek_rate, modified_homing_pulloff;
  float z_pulloff_division_factor = settings.steps_per_mm[Z_AXIS]/settings.steps_per_mm[X_AXIS];
  if (cycle_mask == HOMING_CYCLE_0) {
	  modified_homing_seek_rate = settings.max_rate[Z_AXIS];
	  modified_homing_feed_rate = settings.max_rate[Z_AXIS];
	  //Hardcoding pulloff to 2.5/ the level reduction
	  modified_homing_pulloff = 2.5/settings.lever_reduction;
  }
  else {
	   modified_homing_seek_rate = settings.homing_seek_rate;
	   modified_homing_feed_rate = settings.homing_feed_rate;
	   //Hardcoding for x and y
	   modified_homing_pulloff = 2.5;
  }

  float homing_rate = modified_homing_seek_rate;
  ...
  do {
    ...
    if (approach) {
      max_travel = modified_homing_pulloff *HOMING_AXIS_LOCATE_SCALAR;
      //homing_rate = settings.homing_feed_rate;
	  homing_rate = modified_homing_feed_rate;
    } else {
		max_travel = modified_homing_pulloff;
      //homing_rate = settings.homing_seek_rate;
	  homing_rate = modified_homing_seek_rate;
    }
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    if (cycle_mask & bit(idx)) {
      ...
      #else
        if ( bit_istrue(settings.homing_dir_mask,bit(idx)) ) {
          set_axis_position = lround((settings.max_travel[idx]+modified_homing_pulloff)*settings.steps_per_mm[idx]);
        } else {
          set_axis_position = lround(-modified_homing_pulloff*settings.steps_per_mm[idx]);
        }
      #endif
