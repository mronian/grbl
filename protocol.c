...
// static char debug[8];
static uint8_t crc_in = 0x00;
static uint8_t start_checksum = 0;
static uint8_t char_counter = 0;

char *int2bin(uint8_t a, char *buffer, uint8_t buf_size) {
    buffer += (buf_size - 1);
...
static void protocol_execute_line(char *line)
{
  ...
  // report_echo_line_received(checksum);
  if(start_checksum == 1 && (crc_in!=CRC8(line, char_counter))){
    report_status_message(STATUS_GCODE_ARC_RADIUS_ERROR);
  }
  else if (line[0] == 0) {
  ...
  for (;;) {
    ...
    while((c = serial_read()) != SERIAL_NO_DATA) {
      if ((c == '\n') || (c == '\r')) { // End of line reached
        ...
        crc_in = (checksum[0] - '0')*100 + (checksum[1] - '0')*10 + (checksum[2] - '0');
        ...
        if (comment != COMMENT_NONE) {
          ...
          } else if(start_checksum == 1){
                checksum[checksum_counter++] = c;
          }
